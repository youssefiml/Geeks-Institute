...
async def validate_calendar_request(user_input: str) -> CalendarValidation:
    ...
    try:
        ...
        # Handle content filtering - assume not a calendar request if filtered
        logger.warning(f"Content filtered in calendar validation: {e}")
        return CalendarValidation(is_calendar_request=False, confidence_score=0.0)
    except (RateLimitError, APIError) as e:
        # Handle other API errors - assume not a calendar request if API fails
        logger.error(f"API error in calendar validation: {e}")
        return CalendarValidation(is_calendar_request=False, confidence_score=0.0)
...
async def check_security(user_input: str) -> SecurityCheck:
    ...
    try:
        ...
        # Handle content filtering - if filtered, it's likely unsafe
        logger.warning(f"Content filtered in security check: {e}")
        return SecurityCheck(
            is_safe=False, 
            risk_flags=["Content filtered by API - potential security risk"]
        )
    except (RateLimitError, APIError) as e:
        # Handle other API errors - assume unsafe if API fails
        logger.error(f"API error in security check: {e}")
        return SecurityCheck(
            is_safe=False,
            risk_flags=["API error occurred - cannot verify safety"]
        )
